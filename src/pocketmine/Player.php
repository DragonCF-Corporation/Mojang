						...
						if(trim($message) != "" and strlen($message) <= 255 and $this->messageCounter-- > 0){
							if(substr($message, 0, 2) === "./"){ //Command (./ = fast hack for old plugins post 0.16)
                                                        ...
                                                        if(substr($ev->getMessage(), 0, 1) === "/"){
                                                                ...
                                                                if(!$ev->isCancelled()){
                                                                        $this->server->broadcastMessage($ev->getFormat());
                                                                }
                                                        }
						}
					}
			...
			case EntityDamageEvent::CAUSE_ENTITY_ATTACK:
				if($ev instanceof EntityDamageByEntityEvent){
					...
					if($e instanceof Player){
						$message = $this->getNameTag() . " was killed by " . $e->getName();
						break;
					}elseif($e instanceof Living){
						$message = $this->getNameTag() . " was slain by " . $e->getName();
						break;
					}
				}
				$message = $this->getNameTag() . " was killed";
				break;
			case EntityDamageEvent::CAUSE_PROJECTILE:
				if($ev instanceof EntityDamageByEntityEvent){
					$e = $ev->getDamager();
					if($e instanceof Living){
						$message = $this->getNameTag() . " was shot by " . $e->getName();
						break;
					}
				}
				$message = $this->getNameTag() . " was shot by arrow";
				break;
			case EntityDamageEvent::CAUSE_SUICIDE:
				$message = $this->getNameTag() . " died";
				break;
			case EntityDamageEvent::CAUSE_VOID:
				$message = $this->getNameTag() . " fell out of the world";
				break;
			case EntityDamageEvent::CAUSE_FALL:
				if($ev instanceof EntityDamageEvent){
					if($ev->getFinalDamage() > 2){
						$message = $this->getNameTag() . " fell from a high place";
						break;
					}
				}
				$message = $this->getNameTag() . " hit the ground too hard";
				break;

			case EntityDamageEvent::CAUSE_SUFFOCATION:
				$message = $this->getNameTag() . " suffocated in a wall";
				break;

			case EntityDamageEvent::CAUSE_LAVA:
				$message = $this->getNameTag() . " tried to swim in lava";
				break;

			case EntityDamageEvent::CAUSE_FIRE:
				$message = $this->getNameTag() . " went up in flames";
				break;

			case EntityDamageEvent::CAUSE_FIRE_TICK:
				$message = $this->getNameTag() . " burned to death";
				break;

			case EntityDamageEvent::CAUSE_DROWNING:
				$message = $this->getNameTag() . " drowned";
				break;

			case EntityDamageEvent::CAUSE_CONTACT:
				$message = $this->getNameTag() . " was pricked to death";
				break;

			case EntityDamageEvent::CAUSE_BLOCK_EXPLOSION:
			case EntityDamageEvent::CAUSE_ENTITY_EXPLOSION:
				$message = $this->getNameTag() . " blew up";
				break;

			case EntityDamageEvent::CAUSE_MAGIC:
				$message = $this->getNameTag() . " was slain by magic";
				break;
...
//		}
	...
	public function completeLogin() {
		if(!Player::isValidUserName($this->username)){
		...
		if (!$this->server->isWhitelisted(strtolower($this->getName()))) {
			$this->close(TextFormat::YELLOW . $this->username . " has left the game", "Server in Maintenance.");
			return;
...
//			echo '[INFO] Suiteble item not found in the current inventory.'.PHP_EOL;
	...
	public function addTitle($text, $subtext = '', $time = 1) {
	...
	//						$this->inventory->sendHeldItem($this);
			...
			case -1:  // -1 for 0.16
				...
				if($itemInHand->getId() === Item::SNOWBALL || $itemInHand->getId() === Item::EGG || $itemInHand->getId() === Item::ENDER_PEARL || $itemInHand->getId() === Item::SPLASH_POTION){
					...
					switch ($itemInHand->getId()) {
						case Item::SNOWBALL:
							...
							$this->server->getPluginManager()->callEvent($ev = new ProjectileLaunchEvent($projectile));
							if($ev->isCancelled()){
								$projectile->kill();
							}
							break;
						...
						case Item::SPLASH_POTION:
                                                        $f = 1.1;
                                                        $nbt["PotionId"] = new ShortTag("PotionId", $itemInHand->getDamage());
                                                        $projectile = Entity::createEntity("ThrownPotion", $this->chunk, $nbt, $this);
                                                        $projectile->setMotion($projectile->getMotion()->multiply($f));
                                                        $this->server->getPluginManager()->callEvent($ev = new ProjectileLaunchEvent($projectile));
                                                        if($ev->isCancelled()){
                                                                $projectile->kill();
					...
					if ($projectile instanceof Projectile && $projectile->isAlive()) {
                                                if($this->isSurvival()){
